...
@section('content')
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="{{ route('sales.receipt', ['transaction' => $transaction->id]) }}"
                    class="btn btn-primary shadow-sm rounded-3 px-3" target="_blank">
                ...
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm rounded-3 h-100 d-flex flex-column">
                        ...
                        <div class="card-body p-4 d-flex flex-column justify-content-center flex-grow-1">
                            <table class="table table-borderless mb-0">
                                <tr class="border-top">
                                    <td class="fw-bold text-dark ps-0 py-3 fs-4">Total Amount</td>
                                    <td class="fw-bold text-primary text-end pe-0 py-3 fs-3">
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th class="text-uppercase text-muted small fw-semibold py-3 text-end pe-4">Price</th>
                            </tr>
                        </thead>
                        <tbody class="border-top-0">
                            @foreach($transaction->items as $item)
                                <tr>
                                    ...
                                    <td class="py-3 text-secondary">{{ $item->description }}</td>
                                    <td class="pe-4 py-3 fw-bold text-dark text-end">
                                        ₱{{ number_format($item->pivot->unit_price, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot class="bg-light border-top">
                            <tr>
                                <td colspan="3" class="text-end fw-bold text-uppercase text-secondary py-3">Grand Total:</td>
                                <td class="fw-bold text-primary fs-5 text-end pe-4 py-3">
                                    ₱{{ number_format($transaction->total_amount, 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
